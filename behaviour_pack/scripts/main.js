...
import { world as world3, system, TicksPerSecond } from "@minecraft/server";
...
function load_glitch_component() {
  function glitch(event) {
    ...
    console.log(JSON.stringify(random_location), utils_default.checks.distance_check([location.x, location.z], [random_location.x, random_location.z], radius));
...
import { system as system5, world as world7, TicksPerSecond as TicksPerSecond2 } from "@minecraft/server";
function do_glitch() {
    ...
    switch (glitch) {
      case "noise":
        for (const player of world7.getAllPlayers()) {
          utils_default.commands.noise_glitch(player);
        }
        break;
      case "vision_entity":
        for (const player of world7.getAllPlayers()) {
          utils_default.commands.vision_entity_glitch(player);
        }
        break;
      case "vision_block":
        for (const player of world7.getAllPlayers()) {
          utils_default.commands.vision_block_glitch(player);
        }
        break;
      case "facing":
        break;
